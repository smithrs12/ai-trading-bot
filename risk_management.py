...
class RiskManager:
    ...
    def check_max_drawdown(self) -> bool:
        ...
        try:
            ...
            if cumulative_pnl < -self.max_drawdown_threshold:
                logger.logger.warning(f"⚠️ Max drawdown hit: {cumulative_pnl:.2f}")
                return True

        except Exception as e:
            logger.logger.error(f"❌ Drawdown check failed: {e}")
        return False
    ...
    def update_risk_metrics(self):
        ...
        try:
            ...
            logger.logger.error(f"❌ Risk metric update failed: {e}")
